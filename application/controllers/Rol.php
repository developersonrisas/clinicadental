...
class Rol extends CI_Controller {
    ...
    public function index() {
        ...
        */
        ...
        $orderName = $orderName == null ? 'nombre_rol' : $orderName;
        $orderSort = $this->input->get('orderSort');
        $orderSort = $orderSort == null ? 'asc' : $orderSort;

        $lista = $this->m_rol->mostrar_activos($pageSize, $firstRow, ['r.'.$orderName => $orderSort]); 
        if (!empty($lista)) {
            $data['total'] = count( $this->m_rol->mostrar_activos());
            $i = 1;
